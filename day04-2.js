...
const countValidPassPhrases = (phrases) =>
{
    let counter = 0;

    for (let phrase of phrases)
    {
        if (checkIfValidPhrase(phrase))
        {
            counter = counter + 1;
        }
    }
...
const checkIfValidPhrase = (phrase) =>
{
    const length = phrase.length;

    for (let i = 0; i < length - 1; i++)
    {
        const pass1 = phrase[i];

        for (let j = i + 1; j < length; j++)
        {
            const pass2 = phrase[j];
            ...
            if (checkIfRearrangable(pass1, pass2))
            {
                //console.log(phrase);
...
const checkIfRearrangable = (pass1, pass2) =>
{
    const sortedPass1 = pass1.split('').sort().join("");
    const sortedPass2 = pass2.split('').sort().join("");
    return sortedPass1 === sortedPass2;
};

console.log(countValidPassPhrases(passPhrases));